param(
    [string]$PayloadRoot = "build/runtime-payloads",
...
New-Item -ItemType Directory -Force -Path $Cache,$FfmpegOwned,$TransNet,$PythonStdlib | Out-Null
if ($IncludeSpeech) {
    New-Item -ItemType Directory -Force -Path $Speech,$SpeechOwned,$SpeechModel | Out-Null
}
...
$OwnedRuntimes = @($PythonStdlib, $TransNet)
if ($IncludeSpeech) { $OwnedRuntimes += $SpeechOwned }
foreach ($OwnedRuntime in $OwnedRuntimes) {
...
if ($IncludeSpeech) {
    Write-Host "Windows runtime payload preparation PASSED (including optional 2.0 speech proof payload)." -ForegroundColor Green
}
else {
    Write-Host "Windows runtime payload preparation PASSED (1.0 Core + Editing only; speech deferred)." -ForegroundColor Green
}
