...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run one real Gemini visual-understanding request through the owner chain."
    )
...
def main() -> int:
    ...
    asset = AssetIngestService(
        ...
        created_by="gemini-visual-live-probe",
    )
    ...
    shots = ShotCatalog(
        ...
        (
            ShotBoundaryProposal(
                ...
                detection_method="gemini-visual-live-probe",
            ),
        ),
        created_by="gemini-visual-live-probe",
    )
    ...
    visual_port = GeminiGenerateContentVisualUnderstanding(
        artifact_store=artifact_store,
        transport=transport,
        config=GeminiVisualConfig(model=model, max_output_tokens=1_200),
    )
    ...
    if analysis.visual is None:
        raise RuntimeError("Gemini provider did not produce visual semantics")
    visual = analysis.visual
    if not (visual.summary or visual.tags or visual.subjects or visual.actions):
        raise RuntimeError("Gemini provider returned no searchable visual semantics")
    ...
    print("Gemini visual provider live probe: PASS")
    print(f"model={model}")
