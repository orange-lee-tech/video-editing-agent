...
class DeepSeekScriptProposalReviewPort(ScriptProposalReviewPort):
    ...
    def __init__(
        ...
        *,
        ...
        self._config = DeepSeekChatConfig(max_tokens=2_000) if config is None else config
        ...
        response = self._transport.create_chat_completion(
            {
                ...
                "thinking": {"type": "enabled" if self._config.thinking_enabled else "disabled"},
...
def _parse_violation(value: object, index: int) -> ScriptProposalViolation:
    ...
    return ScriptProposalViolation(
        code=code,
        reason=reason,
        section_id=_optional_review_string(typed["section_id"], field="section_id", index=index),
