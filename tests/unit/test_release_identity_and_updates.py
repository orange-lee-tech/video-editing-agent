...
def _manifest(version: str = "0.1.3") -> str:
    return json.dumps(
        {
            "version": version,
...
def test_update_manifest_parses_and_reports_newer_version() -> None:
    manifest = parse_update_manifest(_manifest("0.1.3"))
    result = UpdateCheckResult("0.1.2", manifest)
...
def test_update_check_is_fail_open_and_does_not_raise(monkeypatch) -> None:
    ...
    result = check_for_update(current_version="0.1.2")
...
def test_update_check_accepts_valid_public_manifest_without_credentials(monkeypatch) -> None:
    ...
    result = check_for_update(current_version="0.1.2", timeout_seconds=1.5)
