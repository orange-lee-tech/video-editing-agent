...
def _manifest(version: str = "0.1.2") -> str:
    return json.dumps(
        {
            "version": version,
...
def test_release_version_identity_is_0_1_1_and_packaging_mirrors_it() -> None:
...
def test_update_manifest_parses_and_reports_newer_version() -> None:
    manifest = parse_update_manifest(_manifest("0.1.2"))
    result = UpdateCheckResult("0.1.1", manifest)
...
def test_update_check_is_fail_open_and_does_not_raise(monkeypatch) -> None:
    ...
    result = check_for_update(current_version="0.1.1")
...
def test_update_check_accepts_valid_public_manifest_without_credentials(monkeypatch) -> None:
    class Response:
        ...
        def read(self, _limit: int) -> bytes:
            return _manifest("0.1.2").encode("utf-8")
    ...
    result = check_for_update(current_version="0.1.1", timeout_seconds=1.5)
