...
def test_hold_execution_remains_canonical_and_held() -> None:
    ...
    assert evaluate_spatial_crop(plan, MediaTime(19, 20)).left == 10
    assert evaluate_spatial_crop(plan, MediaTime(6, 5)).left == 40
...
def test_linear_execution_midpoint_segments_boundaries_and_clamping() -> None:
    ...
    assert "floor(" in execution.video_filter
    assert evaluate_spatial_crop(plan, MediaTime(0, 1)).left == 10
    assert evaluate_spatial_crop(plan, MediaTime(19, 20)) == PixelCrop(25, 5, 99, 176)
    assert evaluate_spatial_crop(plan, MediaTime(6, 5)) == PixelCrop(40, 10, 99, 176)
    assert evaluate_spatial_crop(plan, MediaTime(29, 20)) == PixelCrop(30, 15, 99, 176)
    assert evaluate_spatial_crop(plan, MediaTime(3, 1)).left == 20
...
def test_linear_rounding_is_exact_rational_half_up() -> None:
    ...
    assert evaluate_spatial_crop(plan, MediaTime(6, 5)) == PixelCrop(11, 1, 99, 176)
